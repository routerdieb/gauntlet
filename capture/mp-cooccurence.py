...
def process_dir(dir_list,path,vocab,window_size):
    for directory_name in dir_list:
        ...
        capturer.save_coocurrences('..\\tmp\\coocurence data\\'+directory_name + '.co')


if __name__ == '__main__':
    print('starting')
    if len(sys.argv) != 5:
        raise ValueError('Please provide vocab , wiki-path and window_size and number of processes')
    pqueue = Queue()
    path = sys.argv[2]
    ...
    for process_id in range(num_processes):
            p = Process(target=process_dir, args=(splitted_dirs[process_id],path,vocab,window_size))
            p.start()
