...
messageParameters = 'provide pathIn pathOut and vocab-path and tagged-vocab path'

def combineAndSeperate(pathIn,pathOut,vocab,vocab2):
    block_size = 20000 # must be int
    blocks_amount = math.ceil(vocab.get_size()/float(block_size))
    blocks_amount2 = math.ceil(vocab2.get_size()/float(block_size))

    #init dicts
    dict_of_dicts = {}
    for i in range(blocks_amount):
        for j in range(blocks_amount2):
            dict_of_dicts[(i,j)] = {}
...
if __name__ == '__main__':
    ...
    combineAndSeperate(pathIn,pathOut,vocab,vocab2)
