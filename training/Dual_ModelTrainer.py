...
class Dual_ModelTrainer:
    def __init__(self,vocab_length,vocab2_length,block_path,vector_size,lr):
    ...
    def prepare(self,basepath,experiment_name):
        ...
        self.con_weights     = self.f.create_dataset("context-weights", (self.vocab2_length, self.vector_size))
        self.weights         = self.f.create_dataset("weights",(self.vector_size,self.vocab_length))
        self.context_bias    = self.f.create_dataset("context-bias", (self.vocab2_length,1))
    ...
    def init_weights(self):
        ...
        iterations2 = math.ceil(self.vocab2_length/self.block_length)
        self.tf_weights,self.tf_con_weights,self.tf_bias, self.tf_con_bias  = \
        [None]*iterations,[None]*iterations2,[None]*iterations,[None]*iterations2
    ...
    def _inner_loss(self,weights,context_weights,bias_mat,con_bias_mat,co_occurences):
        ...
        tmp   = bias_terms + weight_matrix - log_X
        tmp   = tf.math.square(tmp)
        tmp   = self.scale_fn(co_occurences) * tmp#elemend wise
